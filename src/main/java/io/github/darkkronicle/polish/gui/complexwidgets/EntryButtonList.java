...
public class EntryButtonList extends AbstractPWidgetList<AbstractPEntry> {
    ...
    public void renderWidget(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        if (background) {
            ...
            DrawUtil.fillRoundedRect(matrices, scaledX, scaledY, scaledWidth, scaledHeight, 9, Colors.DARKGRAY.color().withAlpha(100).color());
            DrawUtil.outlineRoundedRect(matrices, scaledX, scaledY, scaledWidth, scaledHeight, 9, Colors.BLACK.color().withAlpha(200).color());
        }
        matrices.pop();
    }
